...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dontuse_opencv, int dont_show, int mjpeg_port, int calc_topk, int show_imgs, char* chart_path)
{
    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        if (!dontuse_opencv) draw_train_loss(windows_name, "chart.png", img, img_size, avg_loss, max_img_loss, i, net.max_batches, topk, draw_precision, topk_buff, avg_contrastive_acc / 100, dont_show, mjpeg_port, avg_time);
#endif  // OPENCV
